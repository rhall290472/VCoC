...
<body>
    ...
    $protocol = 'https';
    $host = $is_localhost ? ($_SERVER['SERVER_NAME'] ?? 'stanbul') : 'centennialdistrict.co';
    ...
    $teams = [];
    $sql = "SELECT Idx, fullname, Zone FROM teams WHERE TeamType = 'Year Round' ORDER BY fullname";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $teams[$row['Idx']] = ['fullname' => $row['fullname'], 'Zone' => $row['Zone']];
        }
    }

    // Fetch distinct zones for dropdown
    $zones = [];
    $sql = "SELECT DISTINCT Zone FROM teams WHERE TeamType = 'Year Round' ORDER BY Zone";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $zones[] = $row['Zone'];
    ...
    $selectedZone = isset($_POST['filterZone']) && $_POST['filterZone'] !== '' ? $_POST['filterZone'] : null;
    $selectedPosition = isset($_POST['filterPosition']) && $_POST['filterPosition'] !== '' ? $_POST['filterPosition'] : null;

    $sql = "SELECT o.* FROM officals o LEFT JOIN teams t ON o.Team = t.Idx WHERE (o.IsDeleted IS NULL OR o.IsDeleted = 0)";
    $params = [];
    $types = "";
    ...
    if ($selectedPosition !== null) {
        $sql .= " AND o.$selectedPosition > 0";
    }

    $sql .= " ORDER BY o.LastName";
    ...
    <div class="filter-container">
        ...
        <form method="POST" action="">
            ...
            <select name="filterPosition">
                <option value="" <?php echo $selectedPosition === null ? 'selected' : ''; ?>>All Positions</option>
                <?php foreach ($positions as $posKey => $posName): ?>
                    <option value="<?php echo $posKey; ?>" <?php echo $selectedPosition === $posKey ? 'selected' : ''; ?>>
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    <form method="POST" action="">
                        ...
                        <input type="hidden" name="filterZone" value="<?php echo htmlspecialchars($selectedZone); ?>">
                        <input type="hidden" name="filterPosition" value="<?php echo htmlspecialchars($selectedPosition); ?>">
                    </form>
                </tr>
            <?php endwhile; ?>
        <?php else: ?>
